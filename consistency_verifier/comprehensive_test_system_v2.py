...
2. Flexible result parsing (supports both text and JSON)
3. Detailed reporting of differences
4. Ralph Loop integration for fixing issues
...
class MultiwfnInterface:
    ...
    def run(
        self, test_file: Path, commands: List[str], timeout: int = 30
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode != 0:
                raise RuntimeError(
                    f"Multiwfn failed with return code {result.returncode}"
                )
    ...
    def _parse_output(self, output: str) -> Dict[str, Any]:
        """Parse Multiwfn output into structured data."""
...
"""
            ...
            if result.returncode != 0:
                raise RuntimeError(
                    f"PyMultiWFN failed with return code {result.returncode}"
                )
            ...
            raise RuntimeError(f"Could not parse PyMultiWFN JSON output: {str(e)}")
        except Exception as e:
...
class TestRunner:
            ...
            if prop in expected and pymultiwfn_result and mwfn_result:
                ...
                if py_val is not None and mw_val is not None:
                    ...
                    if abs(py_val - mw_val) <= TOLERANCE:
                        ...
                        status = "failed"
                elif pymultiwfn_result is None or mw_val_result is None:
                    diff[prop] = {
                        "status": "missing",
    ...
    def _print_results(self, test_case: TestCase, status: str, diff: Dict[str, Any]):
        ...
        # Print differences
        if diff:
            for prop, result in diff.items():
                result_status = result.get("status", "?")

                if result_status == "match":
                    print(
                        f"  {GREEN}✓{NC} {prop}: {result.get('py_multiwfn')} == {result.get('mwfn')} (diff: {result.get('difference'):.2e})"
                    )
                elif result_status == "mismatch":
                    print(
                        f"  {RED}✗{NC} {prop}: {result.get('py_multiwfn')} != {result.get('mwfn')} (diff: {result.get('difference'):.2e})"
                    )
                elif result_status == "missing":
                    print(
                        f"  {YELLOW}?{NC} {prop}: PyMultiWFN={result.get('py_multiwfn')}, Multiwfn={result.get('mwfn')}"
                    )
...
def main():
    """Main entry point for test system."""

    print(f"{BLUE}{'='*60}\n")

    print(f"{BLUE}{'='*60}\n")
    print(f"{BLUE}Version: 2.0{NC}")
    print(f"{BLUE}{'='*60}\n")
