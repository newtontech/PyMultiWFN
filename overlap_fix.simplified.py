...
def _extract_basis_functions_fixed(wfn, verbose=True):
    ...
    for shell_idx, shell in enumerate(wfn.shells):
        ...
        if shell_type == 0:  # S shell
            ...
            s_coeffs = (
                shell.coefficients[0, :]
                if shell.coefficients.shape[0] == 2
                else shell.coefficients.flatten()
            )
            ...
            if verbose:
                ...
                print(
                    f"Warning: Skipping shell type {shell_type} (not yet implemented)"
                )
...
def _overlap_1d_fixed(i: int, j: int, PA: float, PB: float, p: float) -> float:
    ...
    if i == 2 and j == 2:
        ...
        return (
            PA**2 * PB**2 + (PA**2 + PB**2) / (2 * p) + 1.0 / (4 * p**2) + 3.0 / (4 * p)
        )
    ...
    for i in range(len(exp1)):
        ...
        for j in range(len(exp2)):
            ...
            S_prim = _calculate_primitive_overlap_fixed(
                bf1["type"], bf2["type"], bf1["coords"], bf2["coords"], alpha, beta
            )
