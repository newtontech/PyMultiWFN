...
def create_water_with_normalized_mo():
    ...
    wf.shells = [
        ...
        Shell(type=1, center_idx=0, exponents=np.array([2.0]), coefficients=np.array([[1.0]])),  # O 2p (px, py, pz)
        Shell(type=0, center_idx=1, exponents=np.array([1.0]), coefficients=np.array([[1.0]])),  # H1 1s
        Shell(type=0, center_idx=2, exponents=np.array([1.0]), coefficients=np.array([[1.0]])),  # H2 1s
    ]
    wf.num_shells = 5
...
def analyze_mulliken_issue():
    ...
    print()
    print("  WRONG calculation: sum(PS[bfs_0, all_bfs]) + sum(PS[bfs_1, all_bfs]) = 2 + 1 = 3")
    print("  CORRECT calculation: sum(diag(PS[bfs_0, bfs_0])) + sum(diag(PS[bfs_1, bfs_1])) = 1 + 1 = 2")
    print()
    ...
    print("  # Old (WRONG):")
    print("  # total_atomic_populations[i] = np.sum(PS_tot_element_wise[np.ix_(bfs_i, range(num_basis))])")
    print()
    print("  # New (CORRECT):")
