    ...
    def _process_atom_loop(self, loop_data):
        ...
        for row in data_rows:
            ...
            if "_atom_site_fract_x" in header_map:
                ...
                atom["x"] = (
                    float(row[header_map["_atom_site_Cartesian_x"]]) * ANGSTROM_TO_BOHR
                )
            ...
            if "_atom_site_fract_y" in header_map:
                ...
                atom["y"] = (
                    float(row[header_map["_atom_site_Cartesian_y"]]) * ANGSTROM_TO_BOHR
                )
            ...
            if "_atom_site_fract_z" in header_map:
                ...
                atom["z"] = (
                    float(row[header_map["_atom_site_Cartesian_z"]]) * ANGSTROM_TO_BOHR
                )
            ...
            if "_atom_site_U_iso_or_equiv" in header_map:
                atom["U_iso_or_equiv"] = float(
                    row[header_map["_atom_site_U_iso_or_equiv"]]
                )
            ...
            if all(
                key in atom for key in ["x", "y", "z"]
            ) and self._are_fractional_coords(headers):
