...
class ParserFactory:
    ...
    def _detect_from_content(cls, filename: str) -> Optional[Type]:
        ...
        try:
            ...
            if any(
                line.startswith(("ATOM", "HETATM", "TITLE", "HEADER"))
            ...
            # Check for DX format
            if any(
                "object" in line and ("gridpositions" in line or "field" in line)
                for line in first_lines
            ):
                return DXLoader
            ...
            if any(
                line.startswith("data_") or line.startswith("loop_")
                for line in first_lines
            ):
                return CIFLoader
    ...
    @classmethod
    def load_file(
        cls, filename: str, file_format: Optional[str] = None
    ) -> Wavefunction:
        """
