...
class ORCALoader:
    ...
    def _is_orca_input(self, content: str) -> bool:
        ...
        for line in lines[:10]:  # Check first 10 lines
            ...
            if line.startswith("!"):
                return (
                    True  # ORCA input files start with ! (method/basis specification)
                )
    ...
    def _parse_orca_input(self, content: str):
        ...
        for i, line in enumerate(lines):
            ...
            if line_stripped.upper().startswith(
                "* XYZ"
            ) or line_stripped.upper().startswith("*"):
        ...
        for coord_line in coord_lines:
            ...
            if len(parts) >= 4:
                try:
                    ...
                    self.wfn.add_atom(
                        element, atomic_num, x, y, z, float(atomic_num), atom_info
                    )
                except (ValueError, IndexError):
                    continue
