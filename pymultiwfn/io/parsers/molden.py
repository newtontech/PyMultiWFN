...
class MoldenLoader:
    ...
    def __init__(self, filename: str):
        ...
        if not any(
            ...
            for ext in [".molden", ".molf", ".input", ".inp"]
        ):
            if "molden" not in filename.lower() and not filename.endswith("MOLDEN"):
                warnings.warn(
                    f"File does not appear to be a Molden file: {filename}",
                    RuntimeWarning,
                )
    ...
    def _parse_atoms(self):
        ...
        if atoms_found == 0:
            warnings.warn(
                "No atoms were successfully parsed from [Atoms] section", RuntimeWarning
            )
        else:
    ...
    def _parse_basis(self):
        ...
        for line in gto_section:
            ...
            if len(parts) >= 2 and parts[0].isdigit():
                ...
                if (
                    current_atom is not None
                    and current_shell_type is not None
                    and prim_exps
                ):
                    shell = Shell(
                        ...
                        coefficients=(
                            np.array([prim_coeffs])
                            if current_shell_type != "SP"
                            else np.array(prim_coeffs)
                        ),
                    )
    ...
    def _parse_mo(self):
        ...
        for line in mo_section:
            ...
            if "Ene=" in line or "Occup=" in line or "Spin=" in line:
                ...
                energy_match = re.search(
                    r"Ene\s*=\s*([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)", line
                )
                occup_match = re.search(
                    r"Occup\s*=\s*([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)", line
                )
        ...
        if mo_coeffs_list:
            self.wfn.energies = (
                np.array(energies) if energies else np.zeros(len(mo_coeffs_list))
            )
    ...
    def _validate_parsed_data(self):
        ...
        if len(self.wfn.shells) == 0:
            warnings.warn(
                "No basis shells were parsed from the Molden file", RuntimeWarning
            )
