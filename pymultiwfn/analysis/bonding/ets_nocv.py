...
def calculate_ets_nocv(
    complex_wfn: Wavefunction, fragment_wfns: List[Wavefunction]
) -> Dict[str, Any]:
    """
    ...
    if total_frag_basis != complex_wfn.num_basis:
        raise ValueError(
            f"Total basis functions of fragments ({total_frag_basis}) does not match complex ({complex_wfn.num_basis})."
        )
    ...
    def process_spin(
        ...
        for i, frag_wfn in enumerate(fragment_wfns):
            ...
            coeffs = frag_coeffs_list[
                i
            ]  # (N_basis_frag, N_mo_frag) usually, but check data.py convention
            ...
            for idx in occ_indices:
                ...
                full_vec[current_basis_offset : current_basis_offset + n_basis_frag] = (
                    coeffs[:, idx]
                )
    ...
    if is_unrestricted:
        ...
        evals_a, evecs_a, p_diff_a = process_spin(
            complex_wfn.coefficients,
            complex_wfn.occupations,
        ...
        evals_b, evecs_b, p_diff_b = process_spin(
            complex_wfn.coefficients_beta,
            complex_wfn.occupations_beta,
        ...
        return {"total": {"eigenvalues": evals, "orbitals": evecs, "p_diff": p_diff}}
