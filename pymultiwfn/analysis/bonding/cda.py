...
def calculate_cda(
    ...
    if complex_wfn.num_basis != total_basis_fragments:
        raise ValueError(
            f"Number of basis functions in complex ({complex_wfn.num_basis}) "
    ...
    for i, frag in enumerate(fragments):
        ...
        c_fo_ao[
            current_basis_idx : current_basis_idx + n_basis,
            current_basis_idx : current_basis_idx + n_basis,
        ] = c_frag_T
    ...
    try:
        ...
        raise ValueError(
            "Fragment coefficient matrix is singular. Basis functions might be linearly dependent."
        )
    ...
    if verbose:
        ...
        for item in orbital_data:
            if abs(item["d"]) > 1e-3 or abs(item["b"]) > 1e-3 or abs(item["r"]) > 1e-3:
                print(
                    f"{item['orbital_idx']:<8} {item['occupation']:<8.4f} {item['d']:<12.6f} {item['b']:<12.6f} {item['d']-item['b']:<12.6f} {item['r']:<12.6f}"
                )
        print("-" * 70)
        print(
            f"{'Sum':<8} {'':<8} {total_d:<12.6f} {total_b:<12.6f} {total_d-total_b:<12.6f} {total_r:<12.6f}"
        )

    return {"d": total_d, "b": total_b, "r": total_r, "orbital_terms": orbital_data}
