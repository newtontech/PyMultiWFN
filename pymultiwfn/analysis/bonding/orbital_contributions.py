...
def calculate_orbital_mulliken_contribution(
    wavefunction: Wavefunction, atom1_idx: int, atom2_idx: int
) -> List[Dict[str, Any]]:
    """
    ...
    """
    if (
        wavefunction.coefficients is None
        or wavefunction.overlap_matrix is None
        or wavefunction.occupations is None
    ):
        raise ValueError(
            "Wavefunction coefficients, overlap matrix, or occupations are missing."
        )
    ...
    for imo in range(wavefunction.coefficients.shape[0]):
        occ_alpha = (
            wavefunction.occupations[imo]
            if wavefunction.occupations is not None
            else 0.0
        )
        energy_alpha = (
            wavefunction.energies[imo] if wavefunction.energies is not None else 0.0
        )
        ...
        contrib_alpha = occ_alpha * np.sum(
            np.outer(coeffs_1_mo, coeffs_2_mo) * overlap_12
        )
        ...
        result_entry = {
            ...
            "energy": energy_alpha,
            "contribution_alpha": 2
            * contrib_alpha,  # Fortran multiplies by 2 for the output
    ...
    if (
        wavefunction.is_unrestricted
        and wavefunction.coefficients_beta is not None
        and wavefunction.occupations_beta is not None
    ):
        for imo_beta in range(wavefunction.coefficients_beta.shape[0]):
            occ_beta = (
                wavefunction.occupations_beta[imo_beta]
                if wavefunction.occupations_beta is not None
                else 0.0
            )
            energy_beta = (
                wavefunction.energies_beta[imo_beta]
                if wavefunction.energies_beta is not None
                else 0.0
            )
            ...
            contrib_beta = occ_beta * np.sum(
                np.outer(coeffs_1_mo_beta, coeffs_2_mo_beta) * overlap_12
            )
...
def calculate_orbital_perturbed_mayer_bond_order(
    ...
    (
        unperturbed_mayer_total_matrix,
        unperturbed_mayer_alpha_matrix,
        unperturbed_mayer_beta_matrix,
    ) = calculate_mayer_bond_order(wavefunction)

    before_pert_total = unperturbed_mayer_total_matrix[atom1_idx, atom2_idx]
    before_pert_alpha = (
        unperturbed_mayer_alpha_matrix[atom1_idx, atom2_idx]
        if unperturbed_mayer_alpha_matrix is not None
        else 0.0
    )
    before_pert_beta = (
        unperturbed_mayer_beta_matrix[atom1_idx, atom2_idx]
        if unperturbed_mayer_beta_matrix is not None
        else 0.0
    )
    ...
    for imo in range(wavefunction.coefficients.shape[0]):
        ...
        if occ < 1e-10 and not (
            wavefunction.is_unrestricted
            and wavefunction.occupations_beta is not None
            and imo < wavefunction.coefficients_beta.shape[0]
            and wavefunction.occupations_beta[imo] > 1e-10
        ):
        ...
        if not wavefunction.is_unrestricted:  # Closed-shell
            if occ > 1e-10:
                ...
                perturbed_ptot -= occ * np.outer(
                    mo_coeffs, mo_coeffs
                )  # This should be `occ` times 2 for a doubly occupied orbital in restricted calc.
            ...
            # Alpha part
            if (
                imo < wavefunction.coefficients.shape[0]
                and wavefunction.occupations[imo] > 1e-10
            ):
                mo_coeffs_alpha = wavefunction.coefficients[imo, :]
                perturbed_palpha -= wavefunction.occupations[imo] * np.outer(
                    mo_coeffs_alpha, mo_coeffs_alpha
                )

            # Beta part (if corresponds to a beta MO for the current `imo` context)
            if (
                wavefunction.coefficients_beta is not None
                and wavefunction.occupations_beta is not None
                and imo < wavefunction.coefficients_beta.shape[0]
                and wavefunction.occupations_beta[imo] > 1e-10
            ):
                mo_coeffs_beta = wavefunction.coefficients_beta[imo, :]
                perturbed_pbeta -= wavefunction.occupations_beta[imo] * np.outer(
        ...
        if wavefunction.is_unrestricted:
            perturbed_wf.Ptot = (
                perturbed_palpha + perturbed_pbeta
            )  # Update Ptot if alpha/beta were perturbed

        (
            perturbed_mayer_total_matrix,
            perturbed_mayer_alpha_matrix,
            perturbed_mayer_beta_matrix,
        ) = calculate_mayer_bond_order(perturbed_wf)

        after_pert_total = perturbed_mayer_total_matrix[atom1_idx, atom2_idx]
        after_pert_alpha = (
            perturbed_mayer_alpha_matrix[atom1_idx, atom2_idx]
            if perturbed_mayer_alpha_matrix is not None
            else 0.0
        )
        after_pert_beta = (
            perturbed_mayer_beta_matrix[atom1_idx, atom2_idx]
            if perturbed_mayer_beta_matrix is not None
            else 0.0
        )
        ...
        results.append(
            {
                ...
                "occupation": occ,
                "energy": energy,
                "bond_order_after_perturbation_alpha": (
                    after_pert_alpha if wavefunction.is_unrestricted else None
                ),
                "bond_order_after_perturbation_beta": (
                    after_pert_beta if wavefunction.is_unrestricted else None
                ),
                "bond_order_after_perturbation_total": after_pert_total,
                "variance_total": variance_total,
                "variance_alpha": (
                    variance_alpha if wavefunction.is_unrestricted else None
                ),
                "variance_beta": (
                    variance_beta if wavefunction.is_unrestricted else None
                ),
            }
        )
