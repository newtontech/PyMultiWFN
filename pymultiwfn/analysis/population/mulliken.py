...
def calculate_mulliken_population_and_charges(
    wavefunction: Wavefunction, overlap_matrix: np.ndarray
) -> Tuple[
    np.ndarray,
    np.ndarray,
    Optional[np.ndarray],
    ...
    total_atomic_charges = (
        np.array([atom.charge for atom in wavefunction.atoms])
        - total_atomic_populations
    )
    ...
    return (
        total_atomic_populations,
        total_atomic_charges,
        alpha_atomic_populations,
        beta_atomic_populations,
        spin_densities,
    )
