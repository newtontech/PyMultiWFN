...
def generate_grid(
    min_coords: np.ndarray, max_coords: np.ndarray, spacing: float
) -> np.ndarray:
...
def write_cube(
    filename: str, wfn, grid_points: np.ndarray, density: np.ndarray
) -> None:
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            f"{len(wfn.atoms):5d}{origin[0]:12.6f}{origin[1]:12.6f}{origin[2]:12.6f}\n"
        )
        ...
        for atom in wfn.atoms:
            f.write(
                f"{atom.index:5d}{atom.charge:12.6f}{atom.x:12.6f}{atom.y:12.6f}{atom.z:12.6f}\n"
            )
        # Write density values, 6 per line as per cube format
        values = density.reshape(
            (nx, ny, nz), order="F"
        )  # Fortran order matches generate_grid
