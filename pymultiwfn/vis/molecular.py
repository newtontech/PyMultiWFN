...
class MolecularVisualizer:
    ...
    def __init__(
        self, wavefunction: Wavefunction = None, config: Optional[Config] = None
    ):
        """
    ...
    def create_matplotlib_3d(
        self, show_bonds: bool = True, show_labels: bool = True, atom_style: str = "CPK"
    ) -> plt.Figure:
        """
    ...
    def create_plotly_3d(
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                mode="markers",
                marker=dict(
                    size=sizes, color=colors, line=dict(width=1, color="black")
                ),
    ...
    def get_molecular_properties(self) -> Dict:
        ...
        rg = np.sqrt(
            np.sum(atom_masses[:, np.newaxis] * centered_coords**2) / total_mass
        )
