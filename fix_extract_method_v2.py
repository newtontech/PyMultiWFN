...
for i, line in enumerate(lines):
    if "def _extract_wfn_basis_functions(self) -> List[dict]:" in line:
        start_idx = i
    elif start_idx is not None and line.strip() == "return basis_functions":
        end_idx = i + 1  # Include the return line
        break

if start_idx is None or end_idx is None:
    print("ERROR: Could not find method")
    exit(1)

print(f"Found method at lines {start_idx+1} to {end_idx}")

# The new method implementation (with proper indentation)
...
print(
    f"Successfully replaced _extract_wfn_basis_functions method (lines {start_idx+1} to {end_idx} replaced with {len(new_method.splitlines())} lines)"
)
