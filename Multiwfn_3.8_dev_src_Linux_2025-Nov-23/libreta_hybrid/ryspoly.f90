...
module ryspoly
implicit none
    integer,parameter :: MaxN = 13
    integer,parameter :: MaxN2 = MaxN+2
contains
    subroutine calcryspoly_123(n, X, roots, weights)        
        integer,intent(in) :: n
        real*8,intent(in) :: X
        real*8,intent(out) :: roots(n)
        real*8,intent(out) :: weights(n)        
        real*8 :: R12, PIE4, R22, W22, R13, R23, W23, R33, W33
        real*8 :: F1, F2, E, T1, T2, T3, A1, A2, Y
        real*8 :: RT1, RT2, RT3, WW1, WW2, WW3
        ! ================================================
        RT1 = 0
        RT2 = 0
        RT3 = 0
        ...
        if (X < 3.E-7) then        
            ...
            case (2)
                ...
                RT2 = ((( 3.39024225137123E-04*X-9.34976436343509E-02)*X - &
                        ...
                        1.56184800325063E+02)/X+8.00839033297501E+00)*E + &
                    R22/(X-R22)
                WW2 = ((F1-WW1)*RT1+F1)*(1.0E+00+RT2)/(RT2-RT1)
                WW1 = WW1-WW2
            case (3)
    ...
    subroutine calcryspoly_4(X, roots, weights)        
        real*8,intent(in) :: X
        real*8,intent(out) :: roots(4)
        real*8,intent(out) :: weights(4)        
        real*8 ::  R14, PIE4, R24, W24, R34, W34, R44, W44
        real*8 :: Y, E
        real*8 :: RT1, RT2, RT3, RT4, WW1, WW2, WW3, WW4
        ! ================================================
        RT1 = 0
        RT2 = 0
        RT3 = 0
    ...
    subroutine calcryspoly_5(X, roots, weights)        
        real*8,intent(in) :: X
        real*8,intent(out) :: roots(5)
        real*8,intent(out) :: weights(5)        
        real*8 :: R15, PIE4, R25, W25, R35, W35, R45, W45, R55, W55
        real*8 :: E, Y, XXX
        real*8 :: RT1, RT2, RT3, RT4, RT5, WW1, WW2, WW3, WW4, WW5
        ! ================================================
        RT1 = 0
        RT2 = 0
        RT3 = 0
    ...
    subroutine calcryspoly_ge6(n, X, roots, weights)        
        integer :: n
        real*8,intent(in) :: X
        real*8,intent(out) :: roots(n)
        real*8,intent(out) :: weights(n)
        real*16 :: Sigma(MaxN2*2+1), Sigma0(MaxN2*MaxN2), Mu(MaxN2*MaxN2)
        real*16 :: r(MaxN2*MaxN2), w(MaxN2*MaxN2), rt(MaxN2), A(MaxN2)
        real*16 :: w0, tm, root, temp, poly
        integer :: i, j, k, m
        ! ================================================
        if (n > MaxN) then
            write (*,*) "libreta: Failed in calcryspoly_ge6."
            stop
        endif
        if (n < 2) then 
            n = 2
        endif
        call incompletegamma(n*2, X, Sigma)
        do i = 1, n+1, 1
            do j = 1, n+1, 1
                Sigma0((i-1)*MaxN2+j) = Sigma(i-1+j)
            enddo
        enddo
        call rys_smit(Sigma0, n+1, Mu)             
        r = 0
        w = 0
        w0 = Sigma(1)
        w(1) = w0
        r(1) = Sigma(2)/w0        
        tm = sqrt16(Mu(2*MaxN2+1+1)*Mu(2*MaxN2+1+1)-4*Mu(2*MaxN2+0+1)*Mu(2*MaxN2+2+1))
        r(MaxN2+1) = (-Mu(2*MaxN2+1+1)-tm)/Mu(2*MaxN2+2+1)/2
        r(MaxN2+2) = (-Mu(2*MaxN2+1+1)+tm)/Mu(2*MaxN2+2+1)/2
        if (n /= 2) then        
            rt(1) = r(MaxN2+1)
            rt(2) = r(MaxN2+2)    
            do i = 3, n+1, 1
        ...
        do k = 1, n, 1
            do i = 1, k+1, 1
                ...
                do j = 1, k+1, 1
                    ...
                    do m = 1, j, 1
                        poly = poly*root+Mu(j-m+j*MaxN2+1)
                    enddo
                    temp = temp+poly*poly
                enddo
                w(i+k*MaxN2) = 1/temp
        ...
        enddo
        do i = 1, n, 1
            roots(i) = r(i+(n-1)*MaxN2)     
            weights(i) = w(i+(n-1)*MaxN2)
        enddo
    end subroutine
    function sqrt16(x)
        real*16,intent(in) :: x
    ...
    subroutine incompletegamma(m, t0, f)
        integer,intent(in) :: m
        real*8,intent(in) :: t0
        real*16 :: t
        real*16,intent(out) :: f(m+1)
        real*16,parameter :: SML_FLOAT64 = 1.0E-16
        real*16,parameter :: PI2 = 0.8862269254527579409597138
        real*16 :: b, x, s, e, sqrtt
        integer :: i
        ! ================================================
        t = t0
        if (t < m+1.5) then
            ...
            s = 1
            e = 0.5*exp(-t)
            if (t < SML_FLOAT64) then
                f(m+1) = .5/b
            else
                i = 1
                do while (x > SML_FLOAT64)
                    x = x*t/(b+i)
                    s = s+x
                    i = i+1
                end do
    ...
    subroutine rys_smit(Sigma0, n, Mu)
        real*16,intent(in) :: Sigma0(MaxN2*MaxN2)
        integer,intent(in) :: n
        real*16,intent(out) :: Mu(MaxN2*MaxN2)
        real*16 :: v(MaxN2), fac, dot
        integer :: i, j, k, kmax
        ! ================================================
        Mu = 0
        do j = 1, n, 1
            kmax = j-1
            fac = Sigma0((j-1)*MaxN2+j)    
            if (kmax /= 0) then            
                do k = 1, kmax, 1
                    v(k) = 0
                enddo
                do k = 1, kmax, 1
                    dot = 0
                    do i = 1, k+1, 1
                        dot = dot+Mu((k-1)*MaxN2+i)*Sigma0((j-1)*MaxN2+i)
                    enddo
                    do i = 1, k+1, 1
                        v(i) = v(i)- dot * Mu((k-1)*MaxN2+i)
                    enddo
                    fac = fac-dot*dot
                enddo
            endif
            if (fac < 0) then
                write (*,*) "libreta: Failed in smit."
                stop
            endif
            fac = 1/sqrt16(fac)
            Mu((j-1)*MaxN2+j) = fac            
            do k = 1, kmax, 1
                Mu((j-1)*MaxN2+k) = fac*v(k)
            enddo
        enddo
    end subroutine
    subroutine rys_node(A, k, rt)
        real*16,intent(in) :: A(:)
        integer,intent(in) :: k
        real*16,intent(out) :: rt(:)
        real*16,parameter :: AccRT = 1E-11
        integer,parameter :: MaxIT = 50
        integer :: icount1, icount2, m, i
        real*16 :: r1, r2, r3, r4, r5, r6, p1, p2, p3, p4, p5, p6
        real*16 :: prod, r, dr, delta
        ! ================================================        
        r2 = 0
        p2 = A(1)
        do m = 1, k, 1
            ...
            prod = p1*p2        
            if (prod >= 0) then
                write (*,*) "libreta: Failed in node prod."                
                stop
            endif
            r5 = r1
            p5 = p1
      ...
      30    continue
            ...
            r5 = r-dr
            r6 = r+dr
            if (abs(delta) < AccRT .or. r5 == r .or. r6 == r) then 
                goto 90
            endif
            icount1 = icount1+1
            if (icount1 >= MaxIT) then
                write (*,*) "libreta: Failed in node icount1."
                stop
            endif
            if (r5 < r3) then
                r5 = r3
      ...
      45    continue
            ...
            icount2 = icount2+1
            if (icount2 >= MaxIT) then
                write (*,*) "libreta: Failed in node icount2."
                stop
            endif
            prod = p3*p5
            if (prod > 0.) then 
