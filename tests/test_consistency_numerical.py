...
class TestNumericalPrecision:
    ...
    def test_density_integration_accuracy(self):
        ...
        wfn = Wavefunction(
            ...
            num_shells=2,
            shells=shells,
            occupations=np.array([2.0, 0.0]),  # Bonding orbital fully occupied (2 electrons)
            coefficients=np.array([[coeff, coeff], [coeff, -coeff]]),  # Bonding and antibonding
        )
        ...
        assert 1.5 < total_density < 2.5, f"Integrated density {total_density} far from expected 2 electrons"
    ...
    def test_bond_order_symmetry_precision(self):
        ...
        # Check symmetry
        assert np.allclose(bond_order, bond_order.T, atol=1e-10), \
...
class TestMolecularSystems:
    ...
    def test_h2_bond_order_range(self):
        ...
        shells = [
            Shell(type=0, center_idx=0, exponents=np.array([1.24]), coefficients=np.array([1.0])),
            Shell(type=0, center_idx=1, exponents=np.array([1.24]), coefficients=np.array([1.0])),
        ]
    ...
    def test_n2_triple_bond_order(self):
        ...
        assert n_n_bond > 0 and np.isfinite(n_n_bond), \
...
class TestEdgeCasesNumerical:
    ...
    def test_very_small_overlap(self):
        ...
        assert np.all(np.diag(bond_order) >= 0), "Diagonal bond orders should be non-negative"
