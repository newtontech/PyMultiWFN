...
class TestBondPolarity:
    ...
    def test_homodiatomic_polarity(self, sample_wfn):
        ...
        if wfn.num_atoms >= 2:
            polarity = calculate_bond_polarity(
                'H', 'H',
                wfn.Ptot, wfn.overlap_matrix,
                atom_to_bfs[0], atom_to_bfs[1]
            )
            assert 0.0 <= polarity < 0.1, \
    ...
    def test_polarity_range(self, sample_wfn):
        ...
        for i in range(wfn.num_atoms):
            for j in range(i + 1, wfn.num_atoms):
                polarity = calculate_bond_polarity(
                    wfn.atoms[i].element, wfn.atoms[j].element,
                    wfn.Ptot, wfn.overlap_matrix,
                    atom_to_bfs[i], atom_to_bfs[j]
                )
                assert 0.0 <= polarity <= 1.0, \
        ...
        for i in range(wfn.num_atoms):
            for j in range(i + 1, wfn.num_atoms):
                ...
                wbo_ji = calculate_wiberg_bond_order(
                    wfn.Ptot, wfn.overlap_matrix,
                    atom_to_bfs[j], atom_to_bfs[i]
                )
                assert abs(wbo_ij - wbo_ji) < 1e-10, \
...
class TestIntrinsicBondOrder:
    ...
    def test_ibo_single_bond(self, h2_fchk):
        ...
        assert 0.0 <= ibo <= 1.5, \
            f"H2 IBO should be in [0, 1.5], got {ibo:.3f}"
    ...
    def test_ibo_vs_wiberg(self, sample_wfn):
        ...
        for i in range(bond.natoms):
            for j in range(i + 1, bond.natoms):
                assert ibo_matrix[i, j] <= wiberg_matrix[i, j] + 1e-6, \
    ...
    def test_ibo_matrix_shape(self, sample_wfn):
        ...
        assert result.bond_order_matrix.shape == (bond.natoms, bond.natoms), \
            f"Matrix shape {result.bond_order_matrix.shape} != ({bond.natoms}, {bond.natoms})"
        assert result.polarity_matrix.shape == (bond.natoms, bond.natoms), \
            f"Polarity matrix shape mismatch"
        assert result.wiberg_matrix.shape == (bond.natoms, bond.natoms), \
    ...
    def test_ibo_matrix_symmetric(self, sample_wfn):
        ...
        assert np.allclose(polarity_matrix, polarity_matrix.T), "Polarity matrix not symmetric"
        assert np.allclose(wiberg_matrix, wiberg_matrix.T), "Wiberg matrix not symmetric"
    ...
    def test_ibo_matrix_diagonal_zero(self, sample_wfn):
        ...
        assert np.allclose(np.diag(result.bond_order_matrix), 0.0), \
            "IBO matrix diagonal should be zero"
        assert np.allclose(np.diag(result.wiberg_matrix), 0.0), \
...
class TestBondingClassIntegration:
    ...
    def test_bonding_class_has_ibo_methods(self, sample_fchk):
        ...
        assert hasattr(bond, 'get_intrinsic_bond_order'), \
            "Bonding should have get_intrinsic_bond_order method"
        assert hasattr(bond, 'get_intrinsic_bond_order_matrix'), \
