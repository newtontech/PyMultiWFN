...
class TestOrbitalOverlap:
    ...
    def test_overlap_method_exists(self, h2_wfn):
        ...
        assert hasattr(
            analyzer, "get_orbital_overlap"
        ), "OrbitalsAnalyzer should have get_orbital_overlap method"
    ...
    def test_self_overlap_is_one(self, h2_wfn):
        ...
        for i in range(min(5, n_mo)):  # Test first 5 orbitals
            ...
            assert (
                abs(self_overlap - 1.0) < 0.01
            ), f"Self-overlap for MO {i} should be 1.0, got {self_overlap}"
    ...
    def test_overlap_range(self, h2_wfn):
        ...
        for i in range(min(5, n_mo)):
            for j in range(min(5, n_mo)):
                ...
                assert (
                    -1.01 <= overlap <= 1.01
                ), f"Overlap S({i},{j})={overlap} out of range [-1,1]"
    ...
    def test_overlap_matrix_method(self, c2h4_wfn):
        ...
        ), "Overlap matrix should be numpy array"
        assert overlap_matrix.shape == (
            3,
            3,
        ), f"Overlap matrix should be 3x3, got {overlap_matrix.shape}"
    ...
    def test_overlap_matrix_diagonal_one(self, c2h4_wfn):
        ...
        for i in range(3):
            assert (
                abs(overlap_matrix[i, i] - 1.0) < 0.01
...
class TestOrbitalOverlapAdvanced:
    ...
    def test_orbital_interaction_strength(self, c2h4_wfn):
        ...
        if hasattr(analyzer, "get_interaction_strength"):
            ...
            if lumo_idx < len(analyzer.alpha_energies):
                strength = analyzer.get_interaction_strength(
                    mo_i=homo_idx, mo_j=lumo_idx
                )
                assert isinstance(
                    strength, (int, float)
                ...
                assert (
                    0.0 <= strength <= 1.0
                ), f"Interaction strength should be in [0,1], got {strength}"
