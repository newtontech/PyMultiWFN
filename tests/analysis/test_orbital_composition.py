...
class TestOrbitalComposition:
    ...
    def test_composition_method_exists(self, h2_wfn):
        ...
        assert hasattr(analyzer, 'get_composition'), "OrbitalsAnalyzer should have get_composition method"
    ...
    def test_composition_sums_to_one(self, h2_wfn):
        ...
        for mo_idx in range(min(5, n_mo)):  # Test first 5 MOs
            ...
            total = sum(sum(atom_contrib.values()) for atom_contrib in composition.values())
            assert abs(total - 1.0) < 0.01, f"Composition for MO {mo_idx} should sum to 1.0, got {total}"
    ...
    def test_atomic_contributions_structure(self, h2_wfn):
        ...
        for atom_label, orb_contrib in composition.items():
            assert isinstance(atom_label, str), f"Atom label should be string, got {type(atom_label)}"
            assert isinstance(orb_contrib, dict), f"Orbital contributions should be dict, got {type(orb_contrib)}"
            assert len(orb_contrib) > 0, f"Atom {atom_label} should have orbital contributions"
    ...
    def test_orbital_type_identification(self, c2h4_wfn):
        ...
        assert 's' in orbital_types or 'p' in orbital_types, "C2H4 should have s or p orbital contributions"
    ...
    def test_dominant_orbital_type(self, c2h4_wfn):
        ...
        assert dominant in ['s', 'p', 'd', 'f', 'mixed'], f"Dominant type should be s/p/d/f/mixed, got {dominant}"
    ...
    def test_orbital_localization(self, c2h4_wfn):
        ...
        assert abs(total_localization - 1.0) < 0.01, f"Localization should sum to 1.0, got {total_localization}"
    ...
    def test_multiple_mos_composition(self, c2h4_wfn):
        ...
        if lumo_idx < n_mo:
            ...
            assert homo_comp != lumo_comp, "HOMO and LUMO should have different compositions"
    ...
    def test_ch4_composition(self, ch4_wfn):
        ...
        assert has_carbon or has_hydrogen, "CH4 composition should include C and/or H atoms"
    ...
    def test_composition_numerical_accuracy(self, h2_wfn):
        ...
        for atom_contrib in composition.values():
            for value in atom_contrib.values():
                assert 0.0 <= value <= 1.0, f"Contribution value {value} should be between 0 and 1"
    ...
    def test_degenerate_orbitals(self, c2h4_wfn):
        ...
        if degenerate_pairs:
            for i, j in degenerate_pairs[:1]:  # Test first degenerate pair
                ...
                assert comp_i != comp_j or len(comp_i) > 0, "Degenerate orbitals should have valid compositions"
